...
twosample_mr_function=function(exposure_dat_list=NULL,exposure_name=NULL,
                               outcome_dat_list=NULL,outcome_name=NULL){
  if(is.null(exposure_name)){
    exposure_name=names(exposure_dat_list)
  }
  if(is.null(outcome_name)){
    outcome_name=names(outcome_dat_list)
  }
  all_res<-data.frame()
  all_res_bw=data.frame()
  all_res_bw_detailed=list()
  k=1
  for (i in 1:length(exposure_dat_list)) {
    dat_exp=exposure_dat_list[[i]]
    for (j in 1:length(outcome_dat_list)) {
      ...
      if(!is.null(dim(dat_out))){
        ...
        #main_res
        res <- mr(dat, method_list = c( "mr_ivw","mr_wald_ratio","mr_weighted_median","mr_weighted_mode","mr_egger_regression"))
        res$name_exposure<-rep(exposure_name[i],dim(res)[1])
        res$name_outcome<-rep(outcome_name[j],dim(res)[1])
        
        all_res<-rbind(all_res,res)
        #bwmr
        fit.BWMR <- BWMR(gammahat = dat$beta.exposure,
                         Gammahat = dat$beta.outcome,
                         sigmaX = dat$se.exposure,
                         sigmaY = dat$se.outcome) #run BWMR
        all_res_bw_detailed[[k]]=fit.BWMR
        names(all_res_bw_detailed)[k]=paste0(exposure_name[i],"_",outcome_name[j])
        k=k+1
        
        BWMR_frame_inter=data.frame(beta=fit.BWMR$beta,
                                    se_beta=fit.BWMR$se_beta,
                                    P_value=fit.BWMR$P_value,
                                    tau=fit.BWMR$tau,
                                    sigma=fit.BWMR$sigma,
                                    mu_pi=fit.BWMR$mu_pi,
                                    name_exposure=exposure_name[i],
                                    name_outcome=outcome_name[j])
        all_res_bw=rbind(all_res_bw,BWMR_frame_inter)
        
      }
    }
  }
  all_res_list=list(all_res_main=all_res,all_res_bw=all_res_bw,all_res_bw_detailed=all_res_bw_detailed)
  return(all_res_list)
}

res_all=twosample_mr_function(exposure_dat_list=exposure_dat_list,
                              exposure_name=names(exposure_dat_list),
