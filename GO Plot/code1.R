...
# 读取 Excel 文件
dataC <- read_excel("E:\\Work\\8.27tu\\8.27sixtu\\-log10(p)zhuanhuan\\dataC.xlsx")

# 将 dataC 中的 pvalue 列进行负对数转换，并生成新列 log_p
dataC$log_p <- -log10(dataC$pvalue)



# 截短GOterm列中的文本
dataC$GOterm <- str_trunc(dataC$GOterm, width = 60, ellipsis = "...")
...
# 只保留 "BP"、"MF" 和 "CC" 子组的前10个，更新原始的 dataC 数据框
dataC <- dataC %>%
  group_by(subgroup) %>%
  filter(subgroup == "CC") %>%
  slice_head(n = 10) %>%
  bind_rows(
    dataC %>%
      group_by(subgroup) %>%
      filter(subgroup == "BP" | subgroup == "MF") %>%
      slice_head(n = 10)
  ) %>%
  ungroup()
...
create_plot <- function(data, title, color) {
  ...
  ggplot(data, aes(x = GOterm, y = log_p_adjusted, fill = subgroup)) +
    ...
    scale_y_continuous(
      breaks = c(0, 0.8, 1.8, 2.8),  # 设置实际数据的刻度
      labels = c("1.2", "2", "3", "4")  # 设置显示的标签
    ) +
    coord_cartesian(ylim = c(0, 2.8))  # 使用 coord_cartesian 调整范围
}

# 创建各个子图
plot_BP <- create_plot(subset(dataC, subgroup == "BP"), "BP", "#66AFD8")
plot_CC <- create_plot(subset(dataC, subgroup == "CC"), "CC", "#D86A82")
plot_MF <- create_plot(subset(dataC, subgroup == "MF"), "MF", "#F4A582")
...
# 将 dataC 保存为 Excel 文件
write_xlsx(dataC, path = "E:/Work/8.27tu/8.27sixtu/-log10(p)zhuanhuan/HF_diet-fizzy-drink.xlsx")

# 保存最终图像为PDF格式
ggsave("E:\\Work\\10.3figure\\HF_diet-fizzy-drink.pdf", final_plot, width = 12, height = 8, units = "in", dpi = 600)
