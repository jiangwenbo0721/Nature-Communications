...
# 读取 Excel 文件
dataE <- read_excel("E:\\Work\\8.27tu\\8.27sixtu\\-log10(p)zhuanhuan\\dataE.xlsx")

# 将 dataE 中的 pvalue 列进行负对数转换，并生成新列 log_p
dataE$log_p <- -log10(dataE$pvalue)

# 截短GOterm列中的文本
dataE$GOterm <- str_trunc(dataE$GOterm, width = 60, ellipsis = "...")
...
# 只保留 BP、CC、MF 子组的前 10 个条目
dataE <- dataE %>%
  group_by(subgroup) %>%
  filter((subgroup %in% c("BP", "CC", "MF") & row_number() <= 10)) %>%
  ungroup()
...
create_plot <- function(data, title, color) {
  ...
  ggplot(data, aes(x = GOterm, y = log_p_adjusted, fill = subgroup)) +
    ...
    theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 6),  # 将倾斜角度改为 45 度
    ...
    scale_y_continuous(
      breaks = c(0, 0.8, 1.8, 2.8, 3.8, 4.8, 5.8),  # 设置实际数据的刻度
      labels = c("1.2", "2", "3", "4", "5", "6", "7")  # 设置显示的标签
    ) +
    coord_cartesian(ylim = c(0, 5.8))  # 使用 coord_cartesian 调整范围
}

# 创建各个子图
plot_BP <- create_plot(subset(dataE, subgroup == "BP"), "BP", "#66AFD8")
plot_CC <- create_plot(subset(dataE, subgroup == "CC"), "CC", "#D86A82")
plot_MF <- create_plot(subset(dataE, subgroup == "MF"), "MF", "#F4A582")

# 使用patchwork合并图像
final_plot <- (plot_BP | plot_CC | plot_MF) + 
  plot_layout(ncol = 3, widths = c(1, 1, 1)) +  # 调整宽度比例
...
# 将 dataE 保存为 Excel 文件
write_xlsx(dataE, path = "E:/Work/8.27tu/8.27sixtu/-log10(p)zhuanhuan/T2D_letilsbeans.GO.GO.xlsx")

# 保存最终图像为PDF格式
ggsave("E:\\Work\\10.3figure\\T2D_letilsbeans.GO.GO.pdf", final_plot, width = 12, height = 8, units = "in", dpi = 600)
